...
import { uploadInseePhoto, uploadInsee } from '../../functions/functionsDatabase';
    ...
    try {
      ...
      if (!result.canceled) {
        if (result.assets && result.assets.length > 0 && result.assets[0].uri) {
          ...
          await uploadInseePhoto(uid, result.assets[0].uri);
    ...
    try {
      ...
      if (!result.canceled) {
        if (result.assets && result.assets.length > 0 && result.assets[0].uri) {
          ...
          // Appeler la fonction pour télécharger le document
          await uploadInsee(uid, result.assets[0].uri);
          Alert.alert('Document téléchargé avec succès.');
        } else {
          Alert.alert('Veuillez choisir un document à télécharger.');
        }
      ...
      <ScrollView style={stylesContainers.scrollContainer}>
        <SafeAreaProvider style={stylesContainers.safeProviderContainer}>
          <View style={stylesDocuments.container}>
            <Text style={stylesDocuments.title}>Nous avons besoin de ta déclaration Insee ou de ton extrait Kbis</Text>
